...
def test_youtube_live():
    ...
    assert m.content_type is ContentType.LIVE
...
def test_instagram_story_flags_login_required():
...
def test_instagram_highlight_flags_login_required():
...
def test_twitter_spaces_flags_login_required():
    m = identify("https://x.com/i/spaces/1abcdEFGhijKL")
    assert m.content_type is ContentType.LIVE
    assert m.requires_login_usually is True
